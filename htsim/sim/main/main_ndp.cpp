...
void exit_error(char* progr) {
    cout << "Usage " << progr
         << " [-nodes N]\n\t[-cwnd cwnd_size]\n\t[-q queue_size]\n\t[-oversubscribed_cc] Use "
            ...
            "ar_thresh)]\n\t[-log log_level]\n\t[-seed random_seed]\n\t[-end "
            "end_time_in_usec]\n\t[-mtu MTU]\n\t[-hop_latency x] per hop wire latency in "
    ...
    for (size_t c = 0; c < all_conns->size(); c++) {
        ...
        switch (route_strategy) {
            ...
            case REACTIVE_ECN: {
                ...
                assert(top->switches_lp[topo_cfg->HOST_POD_SWITCH(src)]);
                top->switches_lp[topo_cfg->HOST_POD_SWITCH(src)]->addHostPort(
                    src, ndpSrc->flow_id(), ndpSrc);
                top->switches_lp[topo_cfg->HOST_POD_SWITCH(dest)]->addHostPort(
