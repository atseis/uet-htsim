...
void exit_error(char* progr) {
    cout
        << "Usage " << progr
        << " [-nodes N]\n\t[-cwnd cwnd_size]\n\t[-q queue_size]\n\t[-recv_oversub_cc] Use "
           "receiver-driven AIMD to reduce total window when trims are not last "
           ...
           "ar_thresh)]\n\t[-log log_level]\n\t[-seed random_seed]\n\t[-end "
           "end_time_in_usec]\n\t[-mtu MTU]\n\t[-hop_latency x] per hop wire latency in us,default "
            ...
            switch (route_strategy) {
                ...
                case REACTIVE_ECN: {
                    ...
                    topo[p]->switches_lp[topo_cfg->HOST_POD_SWITCH(src)]->addHostPort(
                        src, eqds_snk->flowId(), eqds_src->getPort(p));
                    topo[p]->switches_lp[topo_cfg->HOST_POD_SWITCH(dest)]->addHostPort(
                        dest, eqds_src->flowId(), eqds_snk->getPort(p));
                    break;
                }
                default:
                    abort();
