...
int main(int argc, char** argv) {
    ...
    topo_cfg = make_unique<FatTreeTopologyCfg>(3, no_of_nodes, linkspeed, queuesize, hop_latency,
    ...
    for (size_t c = 0; c < all_conns->size(); c++) {
        ...
        ((HostQueue*)top->queues_ns_nlp[src][topo_cfg->HOST_POD_SWITCH(src)][0])
        ...
        if (route_strategy != SINGLE_PATH && route_strategy != ECMP_FIB) {
            ...
            srctotor->push_back(
                top->queues_ns_nlp[src][topo_cfg->HOST_POD_SWITCH(src)][0]->getRemoteEndpoint());
            ...
            dsttotor->push_back(
                top->queues_ns_nlp[dest][topo_cfg->HOST_POD_SWITCH(dest)][0]->getRemoteEndpoint());
            ...
            assert(top->switches_lp[topo_cfg->HOST_POD_SWITCH(src)]);
            top->switches_lp[topo_cfg->HOST_POD_SWITCH(src)]->addHostPort(src, hpccSrc->flow_id(),
                                                                          hpccSrc);
            top->switches_lp[topo_cfg->HOST_POD_SWITCH(dest)]->addHostPort(dest, hpccSrc->flow_id(),
                                                                           hpccSnk);
        } else {
