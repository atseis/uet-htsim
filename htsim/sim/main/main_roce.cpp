...
int main(int argc, char** argv) {
    ...
    for (size_t c = 0; c < all_conns->size(); c++) {
        ...
        if (route_strategy != SINGLE_PATH && route_strategy != ECMP_FIB) {
            ...
            assert(top->switches_lp[topo_cfg->HOST_POD_SWITCH(src)]);
            top->switches_lp[topo_cfg->HOST_POD_SWITCH(src)]->addHostPort(src, roceSrc->flow_id(),
                                                                          roceSrc);
            top->switches_lp[topo_cfg->HOST_POD_SWITCH(dest)]->addHostPort(dest, roceSrc->flow_id(),
                                                                           roceSnk);
        } else {
