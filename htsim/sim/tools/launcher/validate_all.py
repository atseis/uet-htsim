...
def check_if_supported_os_ratio(os_ratio):
    if os_ratio not in ["1:1", "4:1", "8:1"]:
        print(
            f"Error: Oversubscription ratio {os_ratio} is not supported. Supported values are: 1:1, 1:4, 1:8"
        )
        exit(1)
...
def check_if_supported_topoogy_size(topology_size):
    if topology_size not in [128, 1024, 8192]:
        print(
            f"Error: Topology size {topology_size} is not supported. Supported values are: 128, 1024, 8192"
        )
        exit(1)
...
def get_cc_name(parameters_experiment):
    if parameters_experiment["cc_algo"] == "rccc":
        ...
        print(
            "Error: Invalid CC Algorithm, supported values are rccc, nscc, nscc+rccc and rccc+os_cc"
        )
        exit(1)
...
def get_topology_file(topology_size, os_ratio):
    ...
    if topology_size == 128:
        return str(resolve_script_path(f"../../htsim/sim/datacenter/topologies/fat_tree_128_{os_ratio}os.topo"))
    elif topology_size == 1024:
        return str(resolve_script_path(f"../../htsim/sim/datacenter/topologies/fat_tree_1024_{os_ratio}os.topo"))
    elif topology_size == 8192:
        return str(resolve_script_path(f"../../htsim/sim/datacenter/topologies/fat_tree_8192_{os_ratio}os.topo"))
...
def get_file_to_run(name_exp, parameters_experiment, global_params, args):
    ...
    if name_exp == "incast":
        ...
        gen_incast_path = resolve_script_path("../../htsim/sim/datacenter/connection_matrices/gen_incast.py")
        cmd_to_run_cm_file = "python {} {} {} {} {} {} 42 1".format(
            gen_incast_path,
            cm_name,
        ...
        cmd_to_run_cm_file = "python {} {} {} {} {} {} 42".format(
        ...
        incast_ratio, outcast_ratio = get_incast_outcast_ratio(
            parameters_experiment["ratio"]
        )
        ...
        gen_outcast_incast_path = resolve_script_path("../../htsim/sim/datacenter/connection_matrices/gen_outcast_incast.py")
        cmd_to_run_cm_file = "python {} {} {} {} {} {} 42".format(
            gen_outcast_incast_path,
            cm_name,
            global_params["topology_sizes"],
            incast_ratio,
            outcast_ratio,
        ...
        gen_allreduce_path = resolve_script_path("../../htsim/sim/datacenter/connection_matrices/gen_allreduce.py")
        cmd_to_run_cm_file = "python {} {} {} {} {} {} 1 42".format(
            gen_allreduce_path,
            cm_name,
            global_params["topology_sizes"],
            global_params["topology_sizes"],
        ...
        gen_allreduce_butterfly_path = resolve_script_path("../../htsim/sim/datacenter/connection_matrices/gen_allreduce_butterfly.py")
        cmd_to_run_cm_file = "python {} {} {} {} {} {} 1 42".format(
            gen_allreduce_butterfly_path,
            cm_name,
            global_params["topology_sizes"],
            1,
            global_params["topology_sizes"],
        ...
        gen_serialn_alltoall_path = resolve_script_path("../../htsim/sim/datacenter/connection_matrices/gen_serialn_alltoall.py")
        cmd_to_run_cm_file = "python {} {} {} {} {} {} {} 0 42".format(
            gen_serialn_alltoall_path,
            cm_name,
            global_params["topology_sizes"],
            global_params["topology_sizes"],
            global_params["topology_sizes"],
            parameters_experiment["parallel_connections"],
            parameters_experiment["message_size_bytes"],
        )
        try:
            # Execute the command
...
def run_experiment(experiment_name, global_params, subparams, args):
    ...
    topo_file = get_topology_file(
        global_params["topology_sizes"], global_params["oversubscription_ratio"]
    )
    ...
    # Launch experiment
    htsim_uec_path = resolve_script_path("../../htsim/sim/build/datacenter/htsim_uec")
    command = "{} -tm {} -o {} -end 1000000 {} -topo {} -linkspeed {} {} {} {} > {}".format(
        htsim_uec_path,
        connection_matrix,
        logout_file,
        cc_algo_to_use,
        topo_file,
        int(global_params["link_speed_Gbps"].replace("Gbps", "")) * 1000,
        disable_os_cc,
        degraded_links,
        args.command_flags,
        output_file,
        ...
        for os_ratio in global_params["oversubscription_ratio"]:
            ...
            for topology_size in global_params["topology_sizes"]:
                ...
                directory = os.path.join(
                    args.output_folder,
                    f"{experiment['name']}_size{topology_size}_osratio{os_ratio}_linkspeed{link_speed}",
                )
                if not os.path.exists(directory):
                    os.makedirs(directory)
                delete_folder_contents(directory)
                directory_tmp = os.path.join(
                    args.output_folder,
                    f"{experiment['name']}_size{topology_size}_osratio{os_ratio}_linkspeed{link_speed}",
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Read and parse a JSON file containing experiments."
    )
    parser.add_argument(
        "--config_json_file", required=True, help="Path to the JSON file"
    )
    parser.add_argument("--show_plot", action="store_true", help="A boolean flag")
    parser.add_argument(
        "--output_folder",
        required=False,
        help="Parent output folder where to save all results",
        default="results",
    )
    parser.add_argument(
        "--command_flags",
        required=False,
        help='Additional command flags to run with each experiment. Include in "", e.g. "-log queue_usage".',
        default="",
    )
