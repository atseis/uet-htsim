...
def run_experiment(experiment_name, global_params, subparams, args):
    ...
    # Launch TCP experiment - algorithm parameter must be first argument
    command = "../htsim_tcp_flex UNCOUPLED -nodes {} -conns {} -tm {} -o {} -end 10000 {} > {}".format(
        global_params["topology_sizes"],
        global_params["topology_sizes"],  # conns should equal nodes for permutation
        connection_matrix,
        logout_file,
        args.command_flags,
        output_file,
...
def main():
    parser = argparse.ArgumentParser(
        description="TCP validation testing framework"
    )
    parser.add_argument(
