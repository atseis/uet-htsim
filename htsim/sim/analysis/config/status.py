...
def get_git_commit() -> str:
    try:
        return subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=Path(__file__).parent.parent.parent.parent).decode('ascii').strip()
    except:
        return "unknown"
...
def update_status(status_file_path: Path, new_status: str, 
    ...
    if error_log: status_data["error_log"] = error_log
