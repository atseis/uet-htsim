...
def _get_available_branches(results_base_dir: Path) -> List[str]:
    """
    [Helper] 获取 results 目录下的所有可用分支目录。

    Args:
        results_base_dir: results 根目录 (不包含分支名层级)
...
def _resolve_result_path_with_branch(
    results_base_dir: Path, rel_path: Path, current_branch: Optional[str] = None
) -> Path:
    """
    [Helper] 解析带分支层级的结果路径。

    策略：
    1. 如果当前分支名已提供，优先使用该分支
    2. 否则尝试查找 results_base_dir 下的所有分支，找到包含对应路径的分支
    3. 如果找不到，降级到旧路径结构（向后兼容）

    Args:
        results_base_dir: results 根目录
        rel_path: YAML 相对路径
        current_branch: 当前分支名（可选）
    ...
    # 策略1: 如果提供了当前分支，直接使用
    if current_branch:
        return results_base_dir / current_branch / rel_path.with_suffix("")

    # 策略2: 扫描所有分支，找到包含该路径的分支
    branches = _get_available_branches(results_base_dir)

    for branch in branches:
        candidate = results_base_dir / branch / rel_path.with_suffix("")
        if candidate.exists():
            return candidate

    # 策略3: 向后兼容 - 尝试旧路径结构（不带分支层级）
    legacy_path = results_base_dir / rel_path.with_suffix("")
    ...
    if branches:
        # 使用第一个找到的分支作为默认
        return results_base_dir / branches[0] / rel_path.with_suffix("")

    # 如果连分支都没有，返回 legacy 路径
    return legacy_path
...
class BatchResult:
    ...
    def __init__(self, current_branch: Optional[str] = None):
        # 存储结构: {unique_uid: {"result": ExperimentResult, "tags": {}}}
        self.experiments: Dict[str, Dict[str, Any]] = {}
        # 自动推断项目根目录
        from ..runner import PROJECT_DIR, get_git_branch_or_commit

        # [New] 存储纯 results 根目录（不含分支层级）
        self._results_root_dir = (PROJECT_DIR / "results").resolve()
        self.experiments_dir = (PROJECT_DIR / "experiments").resolve()

        # [New] 如果没有提供分支名，尝试获取当前分支
        if current_branch is None:
            current_branch = get_git_branch_or_commit()
    ...
    def get_best_experiment(
        self, metric: str = "p99_fct", maximize: bool = False
    ...
    def _load_from_yaml(self, yaml_path: Path, tags: Dict, filters: Dict = None):
        ...
        try:
            ...
            # [New] 使用分支感知的解析函数
            exp_root = _resolve_result_path_with_branch(
                self._results_root_dir, rel_path, self._current_branch
            )
        except ValueError:
            # 如果 YAML 不在 experiments 目录下，降级使用文件名
            exp_root = self._results_root_dir / self._current_branch / yaml_path.stem
    ...
    def _register(self, path: Path, tags: Dict, filters: Dict = None) -> bool:
        ...
        # 生成唯一 ID (使用相对路径，相对于分支目录)
        # [New] 路径现在是相对于 _results_root_dir/{branch} 的
        try:
            # 计算相对于 results/{branch} 的路径
            # path 结构是 results/{branch}/subdir/experiment_name
            # 需要找到 {branch} 部分
            rel_to_root = path.relative_to(self._results_root_dir)
            # rel_to_root 现在应该是 {branch}/subdir/experiment_name
            # 我们需要跳过第一级 (branch) 来生成稳定的 uid
            parts = rel_to_root.parts
            if len(parts) > 1:
                uid = str(Path(*parts[1:]))  # 去掉 branch 层级
            else:
                uid = str(rel_to_root)
        except ValueError:
            # 如果无法解析，使用完整路径
    ...
    def suggest(self) -> Union[pd.DataFrame, "pd.io.formats.style.Styler"]:
        ...
        try:
            ...
            factor_counts = {
                f: sample_df[f].nunique() for f in factors if f in sample_df.columns
            }
        except Exception:
        ...
        if self.experiments:
            ...
            if "flow_events" in logs:
                ...
                available_metrics.extend(
                    ["p99_fct", "median_fct", "max_fct", "avg_fct", "max_slowdown"]
                )
            ...
            if "queue_usage" in logs:
                ...
                available_metrics.extend(
                    ["max_drop_rate", "avg_utilization", "max_q_depth"]
                )
        ...
        primary_metric = (
            metrics_groups["FCT"][0] if metrics_groups["FCT"] else "p99_fct"
        )
        ...
        fct_group_str = (
            str(metrics_groups["FCT"])
            if metrics_groups["FCT"]
            else "['p99_fct', 'max_fct']"
        )
        ...
        if len(factors) == 0:
            ...
            target_metric_list = (
                fct_group_str if metrics_groups["FCT"] else f"['{primary_metric}']"
            )
            ...
            if pd.api.types.is_numeric_dtype(
                sample_df[f1]
            ) and pd.api.types.is_numeric_dtype(sample_df[f2]):
                suggestions.append(
                    {
    ...
    def scan_for_deadlocks(self) -> pd.DataFrame:
        ...
        print(
            f"⚠️ Found {len(df)} deadlock events across {df['_uid'].nunique()} experiments."
        )
        ...
        for _, row in df.iterrows():
            ...
            # 检测逻辑
            if (
                row.get("p99_fct", 0) > THRESH_BLOAT_LATENCY
                and row.get("max_drop_rate", 0) < 0.001
            ):
...
class BatchVisualizer:
        ...
        **sns_kwargs,
        ...
        if kind == "heatmap":
            ...
            def draw_heatmap(data, **kws):
                ...
                try:
                    ...
                    sns.heatmap(
                        pivot_data, annot=True, fmt=".2g", cmap="viridis", cbar=True
                    )
        ...
        # [Mode: Standard Plots]
            ...
            if isinstance(y, list):
                ...
                plot_data = df.melt(
                    id_vars=id_vars, value_vars=y, var_name="Metric", value_name="Value"
                )

                plot_y = "Value"
                if hue:
                    plot_style = (
                        "Metric"  # Hue used by user, differentiate metrics by style
                    )
                else:
            ...
            for ax in g.axes.flat:
                ...
                if (
                    not y2
                ):  # If dual axis, primary formatting might conflict visually if simple
        ...
        if ref_line is not None:
            for ax in g.axes.flat:
                ...
                ax.axhline(
                    y=ref_line, color="red", linestyle=":", linewidth=1.5, alpha=0.8
                )
    ...
    def plot_facet(
        ...
        **kwargs,
        ...
        # 3. 创建分面网格
        g = sns.FacetGrid(
            df, col=col, row=row, sharex=sharex, sharey=sharey, height=4, aspect=1.2
        )
    ...
    def plot_main_effects(
        ...
        for row_idx, group_name in enumerate(group_names):
            ...
            for col_idx, factor in enumerate(factors):
                ...
                for m_idx, m in enumerate(current_metrics):
                    ...
                    try:
                        grouped = (
                            df.groupby(factor, observed=False)[m].agg(agg).reset_index()
                        )
                        sns.lineplot(
                            data=grouped,
                            x=factor,
                ...
                if col_idx == 0:
                    ax.set_ylabel(
                        f"{group_name} ({agg})", fontsize=11, fontweight="bold"
                    )
                else:
                    ax.set_ylabel("")
        ...
        _fig = plt.gcf()  # capture before plt.show() resets state
        plt.show()
        self._attach_save_btn(
            _fig, f"main_effects_{metric if isinstance(metric, str) else metric[0]}"
        )
    ...
    def plot_parallel_coordinates(
            ...
            for col in factors + metrics:
                ...
                if not pd.api.types.is_numeric_dtype(plot_df[col]):
                    ...
                    try:
                        ...
                        print(
                            f"[Warn] Failed to encode column '{col}': {e}. Dropping from plot."
                        )
        ...
        for c in cols_to_plot:
            try:
                ...
                print(
                    f"[Error] Column '{c}' is still non-numeric after encoding. Dropping."
                )
        ...
        with plt.style.context("seaborn-v0_8-whitegrid"):
            ...
            if highlight:
                ...
                if target_metric in df.columns:
                    ...
                    q_val = df[target_metric].quantile(
                        1.0 - fraction if is_top else fraction
                    )
            ...
            if bg_data is not None and len(bg_data) > 0:
                ...
                lc_bg = LineCollection(
                    bg_segs, colors="#d3d3d3", alpha=0.1, linewidths=1
                )
            ...
            if len(hl_data) > 0:
                ...
                lc_hl = LineCollection(
                    hl_segs, colors=colors, alpha=0.8, linewidths=1.5
                )
    ...
    def plot_scatter_matrix(
        ...
        axs = scatter_matrix(df, figsize=(3 * len(cols), 3 * len(cols)), diagonal="kde")
        for ax_row in axs:
            for ax in ax_row:
    ...
    def plot_response_surface(
        ...
        try:
            ...
            print(
                f"[Warn] Response Surface requires numeric X/Y axes. Function aborted.\n    Got types: {x}={plot_df[x].dtype}, {y}={plot_df[y].dtype}"
            )
            return
        ...
        with plt.style.context("seaborn-v0_8-white"):
            ...
            try:
                ...
                print(
                    f"[Warn] Triangulation failed ({e}), falling back to Scatter Plot."
                )
            ...
            def fmt_func_z(val, pos, col=z):
                return self._smart_format(col, val)
            ...
            try:
                ax.tricontour(
                    triang, Z, levels=levels, colors="white", linewidths=0.5, alpha=0.3
                )
            except Exception:
                pass  # 忽略轮廓线错误
            ...
            def fmt_func_x(val, pos, col=x):
                return self._smart_format(col, val)

            def fmt_func_y(val, pos, col=y):
                return self._smart_format(col, val)
    ...
    def plot_auto_interactions(
        ...
        if "ecn_low" in varying and "ecn_high" in varying:
            print(
                f"[Auto-Analysis] Detected ECN Dual-Thresholds. Plotting Interaction for {target_metric}..."
            )
        ...
        if "conns" in varying and "queue_size_bdp_factor" in varying:
            print(
                f"[Auto-Analysis] Detected Load vs Buffer. Plotting Interaction for {target_metric}..."
            )
            self.plot_response_surface(
                x="conns",
        ...
        if "linkspeed" in varying and "hop_latency" in varying:
            print(
                f"[Auto-Analysis] Detected BDP Factors. Plotting Interaction for {target_metric}..."
            )
            self.plot_response_surface(
                x="linkspeed",
                y="hop_latency",
    ...
    def analyze_feature_importance(
        ...
        feature_imp = pd.DataFrame(
            {"Factor": factors, "Importance": importances}
        ...
        sns.barplot(
            x="Importance", y="Factor", data=feature_imp.head(top_n), palette="viridis"
        )
        ...
        if not feature_imp.empty:
            ...
            print(
                f"You should focus your analysis on '{top_factor}' (e.g. plot main effects for this)."
            )

    def find_best_worst_configs(
        self, metric: str = "p99_fct", n: int = 3, maximize: bool = False
    ):
        """
    ...
    def diagnose_failure_modes(self):
        ...
        stragglers = df[
            (df["max_fct"] > 2 * df["p99_fct"]) & (df["p99_fct"] < 2 * df["median_fct"])
        ]
        ...
        if not stragglers.empty:
            ...
            print(
                "    Action: Use Adaptive Routing (e.g. reactive_ecn) or Packet Spraying."
            )
            print("    Example Cases:")
