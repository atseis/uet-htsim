...
DEFAULT_PLOT_OPTIONS = {
    "title": "Flow Completion Time CDF Comparison",
    "y_label": "CDF",
...
def plot_fct_comparison(base_results_dir: Union[str, Path]):
    ...
    if plot_config_path.is_file():
        ...
        try:
            ...
            print(f"❌ 错误: 加载绘图配置文件 {plot_config_path} 失败: {e}，将使用默认配置。")
    else:
    ...
    for line_config in line_configs:
        ...
        if not log_file.is_file():
            print(f"⚠️ 警告: {log_file} 不存在，跳过数据源 {data_source}")
            continue

        # 使用 line_config 中的 label
        label = line_config["label"]

        # 从日志文件解析流事件并计算 FCT
        print(f"  ▶️ 处理数据源: {label} (日志: {log_file.name})")
        try:
            df_flow_events = flow.parse_flow_events_from_file(log_file.as_posix())
            df_fct = metrics.compute_fct(df_flow_events)
            if not df_fct.empty:
                all_fcts.append(df_fct)
                labels.append(label)
                processed_line_configs.append(line_config) # 记录实际处理的配置
            else:
                print(f"  ⚠️ 警告: {log_file} 未解析到有效的流事件，跳过 {data_source}")
        except Exception as e:
            print(f"  ❌ 错误: 处理 {log_file} 时出错: {e}，跳过 {data_source}")
            continue

    if not all_fcts:
        print("没有找到任何有效的FCT数据进行绘图。")
        return

    print("📊 正在生成 FCT CDF 对比图...")
    # 调用 viz.cdf 中的 plot_multiple_fct_cdf 函数
    ...
    print("✅ FCT CDF 对比图生成完成。")


if __name__ == "__main__":
    # 这是一个示例用法，当脚本直接运行时执行
    # 在实际测试中，我们将使用 test_plot_fct.py 来调用它
    project_root = Path(__file__).parent.parent.parent.parent
    test_dir = project_root / "results" / "SimpleTests" / "oblivious_trim_ecn"
    print(f"--- 内部测试运行 plot_fct_comparison ---")
    plot_fct_comparison(test_dir)
    print(f"--- 内部测试完成 ---")