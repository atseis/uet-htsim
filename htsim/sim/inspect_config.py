...
try:
    ...
    class NetworkCalculator:
        def __init__(self, linkspeed_gbps=100.0, mtu_bytes=4150, base_rtt_us=14.0, queue_factor=1.0):
...
class ConfigInspector:
    ...
    def group_by_physics(self):
        ...
        PHYS_KEYS = ['linkspeed', 'mtu', 'tiers', 'hop_latency', 'queue_size_bdp_factor']
    ...
    def _print_variant_table(self, idx, group_data):
        ...
        p_linkspeed = self._parse_linkspeed(params.get('linkspeed')) or D_LINKSPEED # Mbps
        ...
        # Basic Phys
        table.add_row("Linkspeed", f"{p_linkspeed/1000.0} Gbps", f"Raw: {p_linkspeed} Mbps")
        table.add_row("MTU", f"{p_mtu} Bytes", "")
        table.add_row("Base RTT", f"{calc.rtt:.2f} us", f"{p_tiers}-Tier FatTree Est.")
        table.add_row("BDP", f"{calc.bdp_bytes/1000:.1f} KB", f"{calc.bdp_pkts} Packets")
        table.add_row("Queue Size", f"{calc.queue_size_bytes/1000:.1f} KB", f"{calc.queue_size_pkts} Packets ({p_qfactor}x BDP)")
    ...
    def _parse_linkspeed(self, v):
        ...
        s = str(v)
        if "Gbps" in s: return float(s.replace("Gbps", "")) * 1000.0
        if "Mbps" in s: return float(s.replace("Mbps", ""))
        return float(s)
