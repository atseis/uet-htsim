...
public:
    ...
    FatTreeTopologyCfg(istream& file, mem_b queue_size, queue_type q, queue_type snd);
    FatTreeTopologyCfg(uint32_t tiers,
                       ...
                       queue_type q,
                       queue_type snd);
    FatTreeTopologyCfg(uint32_t no_of_nodes,
                       linkspeed_bps linkspeed,
                       mem_b queuesize,
                       queue_type q);
    FatTreeTopologyCfg(uint32_t no_of_nodes,
    ...
    static unique_ptr<FatTreeTopologyCfg> load(string filename,
                                               mem_b queuesize,
                                               queue_type q_type,
    ...
    */
    ...
    void set_tier_parameters(int tier,
                             int radix_up,
                             int radix_down,
                             mem_b queue_up,
                             mem_b queue_down,
    ...
    void set_ecn_parameters(bool enable_ecn,
    ...
    void set_failed_links(int num_failed_links) { _num_failed_links = num_failed_links; }

    void set_tiers(uint32_t tiers) {
        if (tiers != 0)
            _tiers = tiers;
    }
    uint32_t get_tiers() const { return _tiers; }

    void set_latencies(simtime_picosec src_lp,
    ...
    void set_podsize(int hosts_per_pod) { _hosts_per_pod = hosts_per_pod; }
    ...
    uint32_t HOST_POD_SWITCH(uint32_t src) const { return src / _radix_down[TOR_TIER]; }
...
private:
    void initialize(uint32_t tiers,
                    uint32_t no_of_nodes,
                    linkspeed_bps linkspeed,
                    mem_b queuesize,
                    simtime_picosec latency,
