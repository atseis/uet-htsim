...
std::ostream& operator<<(std::ostream& os, FatTreeTopologyCfg const& m) {
    os << "FatTreeTopologyCfg"
       << " NCORE=" << m.NCORE << " NAGG=" << m.NAGG << " NTOR=" << m.NTOR << " NSRV=" << m.NSRV
       << " NPOD=" << m.NPOD << " tor_switches_per_pod=" << m._tor_switches_per_pod
       << " agg_switches_per_pod=" << m._agg_switches_per_pod << " tiers=" << m._tiers
       << " host_per_pod=" << m._hosts_per_pod << " enabled_ecn=" << m._enable_ecn
       << " enable_ecn_on_tor_downlink=" << m._enable_ecn_on_tor_downlink
       << " ecn_low=" << m._ecn_low << " ecn_high=" << m._ecn_high
       << " num_failed_links=" << m._num_failed_links
       << " failed_link_ratio=" << m._failed_link_ratio << " no_of_nodes=" << m._no_of_nodes
       << " hop_latency=" << m._hop_latency << " switch_latency=" << m._switch_latency
       << " diameter_latency=" << m._diameter_latency << " diameter=" << m._diameter;

    for (uint32_t tier = 0; tier < m._tiers; tier++) {
        cout << " tier=" << tier << " link_latency=" << m._link_latencies[tier]
             << " switch_latencies=" << m._switch_latencies[tier]
             << " bundlesize=" << m._bundlesize[tier]
             << " downlink_speeds=" << m._downlink_speeds[tier] << " oversub=" << m._oversub[tier]
             << " radix_down=" << m._radix_down[tier] << " queue_down=" << m._queue_down[tier];

        if (tier < 2) {
            cout << " radix_up=" << m._radix_up[tier] << " queue_up=" << m._queue_up[tier];
        }
    }
...
FatTreeTopologyCfg::FatTreeTopologyCfg(uint32_t tiers,
...
FatTreeTopologyCfg::FatTreeTopologyCfg(istream& file,
                                       mem_b queue_size,
                                       queue_type q,
                                       queue_type snd)
    : FatTreeTopologyCfg(q, snd) {
...
void FatTreeTopologyCfg::initialize(uint32_t tiers,
                                    uint32_t no_of_nodes,
                                    linkspeed_bps linkspeed,
                                    mem_b queuesize,
                                    simtime_picosec latency,
...
FatTreeTopologyCfg::FatTreeTopologyCfg(uint32_t no_of_nodes,
                                       linkspeed_bps linkspeed,
                                       mem_b queuesize,
                                       queue_type q)
...
void FatTreeTopologyCfg::set_tier_parameters(int tier,
                                             int radix_up,
                                             int radix_down,
                                             mem_b queue_up,
                                             mem_b queue_down,
...
unique_ptr<FatTreeTopologyCfg> FatTreeTopologyCfg::load(string filename,
    ...
    if (file.is_open()) {
        unique_ptr<FatTreeTopologyCfg> cfg =
            make_unique<FatTreeTopologyCfg>(file, queuesize, q_type, sender_q_type);
...
FatTreeTopology::FatTreeTopology(const FatTreeTopologyCfg* cfg,
    ...
    for (uint32_t j = 0; j < _cfg->NTOR; j++) {
        ...
        switches_lp[j] = new FatTreeSwitch(*_eventlist, "Switch_LowerPod_" + ntoa(j),
    ...
    for (uint32_t j = 0; j < _cfg->NAGG; j++) {
        simtime_picosec switch_latency = (_cfg->_switch_latencies[AGG_TIER] > 0)
                                             ...
                                             : _cfg->_switch_latency;
        switches_up[j] = new FatTreeSwitch(*_eventlist, "Switch_UpperPod_" + ntoa(j),
        ...
        for (uint32_t agg = agg_min; agg <= agg_max; agg++) {
            for (uint32_t b = 0; b < _cfg->_bundlesize[AGG_TIER]; b++) {
                ...
                if (_cfg->_tiers == 2 && (agg - agg_min) < _cfg->_num_failed_links) {
                    ...
                    cout << "Failure: US" + ntoa(agg) + "->LS" + ntoa(tor) + "(" + ntoa(b) +
                                ") linkspeed set to "
                ...
                queues_nup_nlp[agg][tor][b]->setName("US" + ntoa(agg) + "->LS" + ntoa(tor) + "(" +
                                                     ntoa(b) + ")");
    ...
    switch (_cfg->_qt) {
        ...
        case AEOLUS_ECN: {
            AeolusQueue* q = new AeolusQueue(
                speed, queuesize, FatTreeSwitch::_speculative_threshold_fraction * queuesize,
                *_eventlist, queueLogger);
            if (!tor || dir == UPLINK || _cfg->_enable_ecn_on_tor_downlink) {
                // don't use ECN on ToR downlinks unless configured so.
        ...
        case ECN_PRIO:
            return new ECNPrioQueue(
                speed, queuesize, queuesize, FatTreeSwitch::_ecn_threshold_fraction * queuesize,
                FatTreeSwitch::_ecn_threshold_fraction * queuesize, *_eventlist, queueLogger);
        case LOSSLESS:
