...
tags:
  - <type>                    # code / config / result / claim / ibis/issues
level: "从指定等级列表中挑选" # 必填：标明笔记的思想深度与适用性层级
---
```

**【级别标注规则机制 (level 属性)】（极其重要）**：所有生成的卡片在 Frontmatter 中**必须**包含 `level` 字段。该字段源自 ZTNotes 分类论纲，请根据当前生成卡片的抽象高度和价值属性，在以下列表中**严格选择唯一契合的 value 进行原模原样填入**：
- `"1.5-🧱Atom"`：纯客观陈述的实验现象复述、或者无主观推测的配置详情（如 `code`, `config`）。
- `"2-🔍Insight"`：对单一实验结果或图片的独立理解（多数 `result` 卡片）。
- `"2.5-💊Solution"`：针对错误或缺陷的具体解法、代码 Patch。
- `"3-🧩Synthesis"`：综合了复数个 `Result` 所归纳出来的初步机制想法。
- `"3.5-📝Report"`：针对完整任务的复盘报告（如包含多个图的 `report`）。
- `"4-🔧Mechanism"`：深入剖析网络原理或仿真机制的核心论断（探索底层代码级因果的深度 `claim` 和 `issue`）。
- `"4.5-📖Manual"`：SOP、框架保姆级运行教程。
- `"5-🔥Principle"`：脱离单一实验，触碰普适性流控法则的原理级洞见。
- `"5.5-📐Standard"`：通用设计规范、脚手架工具。
- `"6.5-🗺️Roadmap"`：顶层试验战略规划（如 `ExpPlan` 探底路线图）。
...
- **内容结构**: 每个 result 文件的正文部分（img 之后）必须包含两个明确章节：
  1. `## 实验配置解析`：由于相同的 config 在不同的 code 版本下可能有不同含义（例如 ECN 阈值的单位变化），必须在此处解析该实验的具体背景、设置、场景定义，并明确指明具体的配置含义。**【默认配置溯源规则】**当涉及“默认设置/阈值”（如 ECN 默认阈值）时，绝不能仅用“默认”二字带过，**必须结合当前对应代码版本查明具体的明确取值并写清**（因为默认值在版本更迭中会发生变化）。
  2. `## 实验结果解读`：结合代码现实、实验配置，对图片展示的图表特征、微观现象、背后的网络机制机理进行深入分析解读。
     - **【客观严谨性与猜想声明规则】**实事求是进行分析，如果给出机制论断（例如“这导致了崩溃的唯一原因”），前提是必须能从现有数据中找到真凭实据作为依据。如果依据不足、只是一种推论，则**务必明确标注为“猜想”或“假设”**（例如：“这暗示了一种猜想/假设：只有极端的流路径哈希冲突……”），绝不可将其伪造成已证实的确定性结论。
     - **【论据与证明过程显式化规则】**如果在图表解构过程中，结合了底层的画图代码逻辑、模拟器的 C++ 源码截断、或者是确凿的数学统计公式进行了深度推理。**必须**在 Result 卡片中专门开辟一个子标题（例如：`### 关键论据与代码溯源` 或 `### 深度推理过程`），并将核心的**源码片段**、**数据计算步骤**等具体证据直接粘贴或引用在内。做到每一个重大现象推演“既有宏观判断，又有底层微观代码层面的硬核铁证”。
     - **【图例指标的源码级释义原则 (Code-Truthing)】**绝对禁止对图表中的专业名词（如 Load Factor, Utilization, Overhead 等）进行“望文生义”或“形而上学”的字面猜测。在分析和构建推论前，**必须**全局检索生成这些图表或数据的计算源码（如 `fileinfo.py`、`autoreport.py`），将其最底层的数学计算公式（例如 `Load Factor = Δcum_arr / Δtime`）或实现机理提取出来，以此来确认该指标真实的物理含义（例如它是否允许超过 100%？它是基于排队还是到达率？）。并在卡片的论点中明确标注该公式。
...
- **命名格式**: `Claim-<序号(3位)>-<简述>.md`
  - **【假说驱动的根因分析论证框架】（强制写作结构规范）**：
  为避免死板地罗列论点和证据，所有 Claim 卡片（尤其是 Level 4 及以上的深度洞见）必须采用“剥茧抽丝”加“底层溯源”的五段式叙事架构，有机融合逻辑演绎与源码铁证：
  1. **背景与异常现象 (Context & Phenomenon)**：快速对齐上下文，明确“案发现场”。简述特定配置下观察到的宏观负面现象（需双链引入典型 Result 图表）。
  2. **假说冲突：直觉推断 vs. 深度怀疑 (Hypotheses Conflict)**：(以下只是举例，不一定只有 A, B，也不一定非得有多种可能，完全可以有更多分支与可能，如果证据确凿，也完全可以直接锁定单一答案)
     - *假说 A（常识推断/表象）*：面对现象的第一直觉或业界常识。
     - *假说 B（深度真因）*：抛出本次分析的核心反直觉论点。
  3. **证据链与排他性推演 (Evidence Chain & Elimination)**：
     - *反证组（排除假说 A）*：引用微观打点数据（`rx_data` 归零、队列空窗等），证明常识推断为何破产。
     - *实锤组（锁定假说 B）*：引用底层追踪或全栈切片图，在现象层面完成证据闭环。
  4. **关键论据与代码溯源 (Code-Truthing)**：这是最硬核的一环。必须贴出产生该现象的**决定性底层代码逻辑、核心配置字典或数学计算公式**（结合 `Code-uet-htsim` 等源码纪要），从“物理法则”的高度彻底锁死假说 B。
  5. **根因洞见与结论 (Root Cause Insight & Conclusion)**：一句话概括现象的本质机制，并拔高其对系统架构选型或学术研究的指导意义。

---

### 5. issue 卡片（#ibis/issues）

- 用于记录和追踪在实验或分析过程中发现的具体问题（**对应单个具体的问题**）。
- **强制标签**: 务必在 frontmatter 及正文中作为标签使用 `#ibis/issues`（而非 `#issue`）。
- **命名格式**: `Issue-<序号(3位)>-<问题简述>.md`
- **内部核心元素**：必须包含以下两个基本元素（均为双冒号键值对形式），如果有长文本可使用多个换行或多个标签：
  - `Q::` 对该问题的详细描述。
  - `A::` 对该问题的回答或解决方案。
- **状态管理**：**不需要自动指定 `status` 状态**。它是人工或者别的系统用于追踪任务进度的，在初次生成提取或归纳问题时，请不要自作主张地写死任务状态。
...
| issue  | `Issue-<NNN>-<问题简述>.md`                                | `Issue-001-RTT在长尾末期断更记录.md`           |
