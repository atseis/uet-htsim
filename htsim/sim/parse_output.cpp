...
int main(int argc, char** argv) {
    ...
    */
    ...
    for (int i = 0; i < numRecords; i++) {
        ...
        if (ascii) {
            RawLogEvent event(timeRec[i], typeRec[i], idRec[i], evRec[i], val1Rec[i], val2Rec[i],
                              val3Rec[i], object_names[idRec[i]]);
    ...
    printf("Mean of lower 10pc (%d entries) is %f Mbps total mean %f Mbps mean2 %f Mbps\n", cnt,
           (total / cnt) * 8 / 1000000, mean_rate / rates.size() * 8 / 1000000,
