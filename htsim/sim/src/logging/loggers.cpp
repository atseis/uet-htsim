...
void QueueLoggerSimple::logQueue(BaseQueue& queue, QueueLogger::QueueEvent ev, Packet& pkt) {
    _logfile->writeRecord(Logger::QUEUE_EVENT, queue.get_id(), ev, (double)queue.queuesize(),
                          pkt.flow().get_id(), pkt.id());
}
...
void QueueLoggerEmpty::doNextEvent() {
    ...
    if (_queue) {
        ...
        cout << eventlist().now() << " " << _queue->nodename() << " " << _total_busy << " "
             ...
             << _queue->queuesize_high_watermark() << endl;
    }
    reset_count();
...
void QueueLoggerSampling::doNextEvent() {
    ...
    if (!_seenQueueInD) {  // queue size hasn't changed in the past D time units
        _logfile->writeRecord(QUEUE_APPROX, _queue->get_id(), QUEUE_RANGE, (double)_lastq,
                              (double)_lastq, (double)_lastq);
        ...
        _logfile->writeRecord(QUEUE_APPROX, _queue->get_id(), QUEUE_OVERFLOW,
                              -(double)_lastIdledInD, (double)_lastDroppedInD, (double)queuebuff);
    }
    _seenQueueInD = false;
...
void FlowEventLoggerSimple::logEvent(PacketFlow& flow,
                                     Logged& location,
                                     FlowEvent ev,
                                     mem_b bytes,
                                     uint64_t pkts) {
...
void TrafficLoggerSimple::logTraffic(Packet& pkt, Logged& location, TrafficEvent ev) {
    _logfile->writeRecord(Logger::TRAFFIC_EVENT, location.get_id(), ev, pkt.flow().get_id(),
                          pkt.id(), 0);
}
...
void TcpTrafficLogger::logTraffic(Packet& pkt, Logged& location, TrafficEvent ev) {
    _logfile->writeRecord(Logger::TCP_TRAFFIC, location.get_id(), ev, pkt.flow().get_id(), pkt.id(),
                          0);
}
...
void SwiftTrafficLogger::logTraffic(Packet& pkt, Logged& location, TrafficEvent ev) {
    _logfile->writeRecord(Logger::SWIFT_TRAFFIC, location.get_id(), ev, pkt.flow().get_id(),
                          pkt.id(), 0);
}
...
void STrackTrafficLogger::logTraffic(Packet& pkt, Logged& location, TrafficEvent ev) {
    _logfile->writeRecord(Logger::STRACK_TRAFFIC, location.get_id(), ev, pkt.flow().get_id(),
                          pkt.id(), 0);
}
...
void NdpTrafficLogger::logTraffic(Packet& pkt, Logged& location, TrafficEvent ev) {
    ...
    _logfile->writeRecord(Logger::NDP_TRAFFIC, location.get_id(), ev, p.flow().get_id(), p.id(),
                          val3);
}
...
void RoceTrafficLogger::logTraffic(Packet& pkt, Logged& location, TrafficEvent ev) {
    ...
    _logfile->writeRecord(Logger::ROCE_TRAFFIC, location.get_id(), ev, p.flow().get_id(), p.id(),
                          val3);
}
...
void HPCCTrafficLogger::logTraffic(Packet& pkt, Logged& location, TrafficEvent ev) {
    ...
    _logfile->writeRecord(Logger::HPCC_TRAFFIC, location.get_id(), ev, p.flow().get_id(), p.id(),
                          val3);
}
...
void TcpLoggerSimple::logTcp(TcpSrc& tcp, TcpEvent ev) {
    ...
    _logfile->writeRecord(Logger::TCP_STATE, tcp.get_id(), TcpLogger::TCPSTATE_CNTRL, tcp._cwnd,
                          tcp._ssthresh, tcp._recoverq);

    _logfile->writeRecord(Logger::TCP_STATE, tcp.get_id(), TcpLogger::TCPSTATE_SEQ, tcp._last_acked,
...
string TcpLoggerSimple::event_to_str(RawLogEvent& event) {
    ...
    switch (event._type) {
        ...
        default:
            ss << " Type " << event._type << " ID " << event._id << " Ev " << event._ev << " VAL1 "
               << event._val1 << " VAL2 " << event._val2 << " VAL3 " << event._val3 << endl;
    }
    return ss.str();
}
void SwiftLoggerSimple::logSwift(SwiftSubflowSrc& swift, SwiftEvent ev) {
    _logfile->writeRecord(Logger::SWIFT_EVENT, swift.get_id(), ev, swift._swift_cwnd,
                          swift._inflate, 0);

    _logfile->writeRecord(Logger::SWIFT_STATE, swift.get_id(), SwiftLogger::SWIFTSTATE_CNTRL,
                          swift._swift_cwnd, 0, swift._recoverq);

    _logfile->writeRecord(Logger::SWIFT_STATE, swift.get_id(), SwiftLogger::SWIFTSTATE_SEQ,
                          swift._last_acked, swift._highest_sent, swift._RFC2988_RTO_timeout);
}
...
void STrackLoggerSimple::logSTrack(STrackSrc& strack, STrackEvent ev) {
    _logfile->writeRecord(Logger::STRACK_EVENT, strack.get_id(), ev, strack._strack_cwnd,
                          strack._inflate, 0);

    _logfile->writeRecord(Logger::STRACK_STATE, strack.get_id(), STrackLogger::STRACKSTATE_CNTRL,
                          strack._strack_cwnd, 0, strack._recoverq);

    _logfile->writeRecord(Logger::STRACK_STATE, strack.get_id(), STrackLogger::STRACKSTATE_SEQ,
                          strack._last_acked, strack._highest_sent, strack._RFC2988_RTO_timeout);
}
...
void AggregateTcpLogger::doNextEvent() {
    ...
    _logfile->writeRecord(Logger::TCP_RECORD, get_id(), TcpLogger::AVE_CWND, totcwnd / numflows,
                          totunacked / numflows, toteffcwnd / numflows);
}
...
void MultipathTcpLoggerSimple::logMultipathTcp(MultipathTcpSrc& mtcp, MultipathTcpEvent ev) {
    if (ev == MultipathTcpLogger::CHANGE_A) {
        ...
        _logfile->writeRecord(
            MultipathTcpLogger::MTCP, mtcp.get_id(), ev, mtcp._subflows.front()->_rtt / 1000000000,
...
string MultipathTcpLoggerSimple::event_to_str(RawLogEvent& event) {
    ...
    switch (event._ev) {
        case MultipathTcpLogger::CHANGE_A:
            ss << " Ev CHANGE_A" << " ID " << event._id << "A " << event._val1 << "Alfa "
               << event._val2;
            break;
...
void MemoryLoggerSampling::doNextEvent() {
    ...
    for (i = 0; i < _tcp_sources.size(); i++) {
        _logfile->writeRecord(Logger::TCP_MEMORY, _tcp_sources[i]->get_id(), TcpLogger::MEMORY, 0,
                              0, _tcp_sources[i]->_highest_sent - _tcp_sources[i]->_last_acked);
    }

#ifdef MODEL_RECEIVE_WINDOW
    for (i = 0; i < _mtcp_sinks.size(); i++) {
        _logfile->writeRecord(Logger::MTCP, _mtcp_sinks[i]->get_id(), MultipathTcpLogger::MEMORY, 0,
                              0, _mtcp_sinks[i]->_received.size() * 1000);
    }

    for (i = 0; i < _mtcp_sources.size(); i++) {
        _logfile->writeRecord(Logger::MTCP, _mtcp_sources[i]->get_id(), MultipathTcpLogger::MEMORY,
                              0, 0,
...
void NicLoggerSampling::doNextEvent() {
    ...
    for (uint64_t i = 0; i < _nics.size(); i++) {
        ...
        _logfile->writeRecord(Logger::NIC_EVENT, _nics[i]->get_id(), EqdsLogger::RATE, new_rate,
                              total_rate, trim_rate);
    }
}
...
string NicLoggerSampling::event_to_str(RawLogEvent& event) {
    ...
    ss << " Data " << (uint64_t)event._val1 << " Total " << (uint64_t)event._val2 << " Trim "
       << (uint64_t)event._val3;
    return ss.str();
}
...
void SinkLoggerSampling::doNextEvent() {
    ...
    for (it = _multipath_src.begin(); it != _multipath_src.end(); it++) {
        ...
        _logfile->writeRecord(Logger::MTCP, mtcp->get_id(), MultipathTcpLogger::RATE, mtcp->a,
                              goodput, throughput);
...
void RoceSinkLoggerSampling::doNextEvent() {
    ...
    for (uint64_t i = 0; i < _sinks.size(); i++) {
        ...
        if (_last_seq[i] <= sink->total_received()) {
            deltaB = sink->total_received() - _last_seq[i];
            deltaB *= Packet::data_packet_size();
            if (delta > 0)
                rate = deltaB * 1000000000000.0 / delta;  // Bps
            else
                rate = 0;
...
void QcnLoggerSimple::logQcn(QcnReactor& src, QcnEvent ev, double var3) {
    if (ev != QcnLogger::QCN_SEND)
        _logfile->writeRecord(Logger::QCN_EVENT, src.get_id(), ev, src._currentRate,
                              src._packetCycles, var3);
}

void QcnLoggerSimple::logQcnQueue(QcnQueue& src,
                                  QcnQueueEvent ev,
                                  double var1,
                                  double var2,
                                  double var3) {
...
string QcnLoggerSimple::event_to_str(RawLogEvent& event) {
    ...
    switch (event._type) {
        case Logger::QCN_EVENT:
            ...
            ss << " Rate " << (int)event._val1 << " PktCycles " << (int)event._val2 << " Val3 "
               << event._val3;
            break;
...
void ReorderBufferLoggerSampling::doNextEvent() {
    ...
    eventlist().sourceIsPendingRel(*this, _period);
    _logfile->writeRecord(QUEUE_APPROX, get_id(), QueueLogger::QUEUE_RANGE, (double)_queue_len,
                          (double)_min_queue, (double)_max_queue);
