...
void NdpSrc::processNack(const NdpNack& nack) {
    ...
    */
    ...
    switch (_route_strategy) {
        ...
        case ECMP_FIB:
        case ECMP_FIB_ECN:
            p = NdpPacket::newpkt(_flow, *_route, seqno, 0, _mss, true, _path_ids.size(),
...
int NdpSrc::send_packet(NdpPull::seq_t pacer_no) {
    ...
    if (!_rtx_queue.empty()) {
        ...
        switch (_route_strategy) {
            ...
            case ECMP_FIB_ECN:
            case REACTIVE_ECN: {
                p = NdpPacket::newpkt(_flow, *_route, _highest_sent + 1, pacer_no, _mss, false,
            ...
            case SINGLE_PATH:
                p = NdpPacket::newpkt(_flow, *_route, _highest_sent + 1, pacer_no, _mss, false, 1,
                                      last_packet, _dstaddr);
                break;
            case NOT_SET:
                abort();
...
void NdpSrc::retransmit_packet() {
    ...
    for (j = rtx_list.begin(); j != rtx_list.end(); j++) {
        ...
        switch (_route_strategy) {
            ...
            case SINGLE_PATH:
                p = NdpPacket::newpkt(_flow, *_route, seqno, 0, _mss, true, _paths.size(),
                                      last_packet, _dstaddr);
                break;
            case NOT_SET:
                abort();
...
void NdpSink::receiver_increase(NdpPacket* p) {
    ...
    if (p->pacerno() >= _parked_increase && _parked_cwnd > (uint32_t)Packet::data_packet_size()) {
        ...
        if (_route)
            ...
            pull_pkt = NdpPull::newpkt(p->flow(), *(_paths[random() % _paths.size()]),
...
void NdpSink::send_ack(simtime_picosec ts,
    ...
    switch (_route_strategy) {
        ...
        case SCATTER_ECMP:
            assert(_paths.size() > 0);
            ack = NdpAck::newpkt(_src->_flow, *(_paths.at(_crt_path)), 0, ackno, _cumulative_ack,
...
void NdpSink::send_nack(simtime_picosec ts,
    ...
    switch (_route_strategy) {
        ...
        case SCATTER_ECMP:
            assert(_paths.size() > 0);
            nack = NdpNack::newpkt(_src->_flow, *(_paths.at(_crt_path)), 0, ackno, _cumulative_ack,
                                   _pull_no, _path_history[_path_hist_index].path_id(), _srcaddr);
            if (_route_strategy == SCATTER_RANDOM) {
                _crt_path = random() % _paths.size();
            } else {
