...
public:
    ...
    public:
        virtual void msg_status_changed(UecPdcSes& pdc,
                                        UecMsg::msgid_t msg_id,
                                        UecMsg::MsgStatus new_status) = 0;
    };
    struct Stats {
     ...
     */
    void setStatusCallback(MsgStatus status, StatusCallback* callback) {
        _callbacks.at(status).emplace(callback);
    };
